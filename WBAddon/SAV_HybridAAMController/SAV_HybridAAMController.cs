...
namespace SaccFlightAndVehicles
{
    ...
    public class SAV_HybridAAMController : UdonSharpBehaviour
    {
        ...
        // === AGM統合機能 ===
        [Header("AGM Integration - Armor System")]
        ...
        [Header("AGM Integration - Knockback")]
        ...
        [Tooltip("Enable AGM-style damage system for ground targets")]
        public bool EnableAGMDamageForGround = true;
        [Tooltip("Enable knockback effects for all targets")]
        public bool EnableKnockbackForAll = false;
        ...
        [Header("=== 武器タイプ ===")]
        [Tooltip("武器タイプID（キル通知用）")]
        public byte WeaponType = 10;
        void Initialize()
        {
        ...
        // === ハイブリッドダメージシステム ===
        private SaccFlightAndVehicles.TargetType DetermineTargetType(SaccEntity targetEntity)
        {
            if (targetEntity == null) return SaccFlightAndVehicles.TargetType.Ground;

            // SaccAirVehicleを持っている場合は航空機と判定
            if (targetEntity.GetComponent<SaccAirVehicle>() != null)
            {
                return SaccFlightAndVehicles.TargetType.Aircraft;
            }

            // 特定のコンポーネントやタグで地上目標を判定
            // ここでは簡易的に、航空機コンポーネントを持たないものを地上目標と判定
            return SaccFlightAndVehicles.TargetType.Ground;
        }
            ...
            if (!Exploding)
            {
                if (IsOwner)
                {
                    ...
                    if (TargetEntity)
                    {
                        ...
                        // ハイブリッドダメージ処理
                        ApplyHybridDamage(TargetEntity, other);
                    }
                }
                hitwater = false;
                Explode();
        ...
        private void ApplyHybridDamage(SaccEntity targetEntity, Collision collision)
        {
            SaccFlightAndVehicles.TargetType targetType = DetermineTargetType(targetEntity);

            if (targetType == SaccFlightAndVehicles.TargetType.Aircraft)
            {
                // 既存のAAMダメージ処理（航空機用）
                ApplyAAMDamage(targetEntity);
            }
            else if (EnableAGMDamageForGround)
            {
                // AGMのダメージシステム（地上目標用）
                ApplyAGMDamage(targetEntity, collision);
            }
            else
            {
                // AGMダメージ無効時はAAMダメージを適用
                ApplyAAMDamage(targetEntity);
            }
        }

        private void ApplyAAMDamage(SaccEntity targetEntity)
        {
            // 既存のAAMダメージ処理
            targetEntity.SendEventToExtensions("SFEXT_L_MissileHit100");
        }

        private void ApplyAGMDamage(SaccEntity targetEntity, Collision collision)
        {
            // AGMの装甲システムを統合
            ...
            {
                string pname = collision.collider.transform.GetChild(0).name;
            ...
            if (DoDamage)
            {
                // WeaponDamageVehicle() メソッドを使用
                targetEntity.WeaponDamageVehicle(dmgLvl, gameObject,WeaponType);
            }
        }
        ...
        private void Explode()
        {
            ...
            // ノックバックシステム（オプション）
            if (EnableKnockbackForAll && KnockbackRadius > 0)
            {
                ApplyKnockback();
            }
